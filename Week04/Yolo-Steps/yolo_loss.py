...
class YoloLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        # Location loss
        loss_x = nn.MSELoss(size_average=False)(x*coord_mask, tx*coord_mask)/2.0
        loss_y = nn.MSELoss(size_average=False)(y*coord_mask, ty*coord_mask)/2.0
        loss_w = nn.MSELoss(size_average=False)(w*coord_mask, tw*coord_mask)/2.0
        loss_h = nn.MSELoss(size_average=False)(h*coord_mask, th*coord_mask)/2.0
        loss_loc = self.coord_scale * (loss_x + loss_y + loss_w + loss_h)
